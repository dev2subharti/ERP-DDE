...
namespace DDE.Web.Admin
{
    public partial class ShowDAApproved : System.Web.UI.Page
    {
        ...
        {
            dtlistShowDegreeInfo.Visible = false;
            //CheckBox chk = (CheckBox)dtlistShowDegreeInfo.FindControl("chkSelect");
            //chk.Visible = false;
        ...
        {
            String varSrid = "";
            for (Int32 i = 0; i <= dtlistShowDegreeInfo.Items.Count - 1; i++)
            {
                CheckBox chk = (CheckBox)dtlistShowDegreeInfo.Items[i].FindControl("chkSelect");
                if (chk.Checked == true)
                {
                    Label varLblSrid = (Label)dtlistShowDegreeInfo.Items[i].FindControl("lblSRID");
                    varSrid += varLblSrid.Text + ",";
                }
            }

            if (varSrid.ToString().Trim().Length > 0)
            {
                Session["Srid"] = varSrid;
