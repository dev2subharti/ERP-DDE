...
{
    public partial class PublishMarkSheetBySC : System.Web.UI.Page
    {
        ...
        private void populateMarks()
        {
            ...
            foreach (DataListItem dli in dtlistMS.Items)
            {
                ...
                SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["CSddedb"].ToString());
                SqlCommand cmd = new SqlCommand("select * from DDESubject where SyllabusSession='A 2010-11' and CourseName='" + FindInfo.findCourseNameBySRID(Convert.ToInt32(srid.Text), Convert.ToInt32(year.Text)) + "' and Year='" + FindInfo.findAlphaYear(year.Text) + "'", con);
                SqlDataReader dr;
                ...
                SqlCommand cmd1 = new SqlCommand("select * from DDEPractical where SyllabusSession='A 2010-11' and CourseName='" + FindInfo.findCourseNameBySRID(Convert.ToInt32(srid.Text), Convert.ToInt32(year.Text)) + "' and Year='" + FindInfo.findAlphaYear(year.Text) + "'", con1);
                SqlDataReader dr1;
        ...
        {
            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["CSddedb"].ToString());
            SqlCommand cmd = new SqlCommand("select * from DDEPracticalMarks_A13 where SRID='" + srid + "' and PracticalID='" + pid + "' and MOE='R'", con);
            SqlDataReader dr;
        ...
        {
                SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["CSddedb"].ToString());
                SqlCommand cmd = new SqlCommand("select * from DDEMarkSheet_A13 where SRID='" + srid + "' and SubjectID='" + subid + "' and MOE='R'", con);
                SqlDataReader dr;
        ...
        private void populateStudents(string students)
        {
            ...
            for (int i=0; i < srid.Length; i++)
            {
                ...
                while (dr.Read())
                {
                    ...
                    drow["RollNo"] = FindInfo.findRollNoBySRID(Convert.ToInt32(srid[i]),"A13","R");
                    drow["EnrollmentNo"] = Convert.ToString(dr["EnrollmentNo"]);
                    drow["Examination"] = "Examination : JUNE 2013";
                    drow["Year"] = findYear(Convert.ToInt32(srid[i]));                 
                    drow["Course"] = FindInfo.findCourseAndYearForMS(Convert.ToInt32(srid[i]), Convert.ToInt32(drow["Year"]));
        ...
        private string  findYear(int srid)
        {
           string year = "0";
