...
{
    public partial class GenerateQuestionPaper : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Authorisation.authorised(Convert.ToInt32(Session["ERID"]), 106))
            {
                if (!IsPostBack)
                {
                    ...
                    ddlistExam.Items.FindByValue("Z10").Selected = true;
        ...
        private void setAccesibility()
        {
            ...
            foreach (DataListItem dli in dtlistShowDS.Items)
            {
                ...
                foreach (DataListItem dlis in subjects.Items)
                {
                    LinkButton set = (LinkButton)dlis.FindControl("lnkbtnSet");
                    LinkButton show = (LinkButton)dlis.FindControl("lnkbtnShow");
                    Button status = (Button)dlis.FindControl("btnStatus");
                    Label paperset = (Label)dlis.FindControl("lblPaperSet");
                   

                    if (paperset.Text == "YES")
                    {
                        set.Visible = false;
                        status.Visible = true;
                        show.Visible = true;
                    }
                    else if (paperset.Text == "NO")
                    {
                        set.Visible = true;
        ...
        private void populateSubjects()
        {
            ...
            foreach (DataListItem dli in dtlistShowDS.Items)
            {
                ...
                DataColumn dtcol9 = new DataColumn("PaperSet");
                ...
                for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (Exam.isQuestionPaperAlreadySet(ddlistExam.SelectedItem.Value, drow["PaperCode"].ToString()))
                    {
                        drow["PaperSet"] = "YES";
                    }
                    else
                    ...
                    if (Exam.isQuestionPaperOnline(ddlistExam.SelectedItem.Value, drow["PaperCode"].ToString()))
                    {
                        drow["Status"] = "ONLINE";
                    }
                    else
        ...
        protected void lnkbtnSet_Click(object sender, EventArgs e)
        {
            ...
            Session["QPFrom"] = Convert.ToInt32(tbFrom.Text);
            Session["QPTo"] = Convert.ToInt32(tbTo.Text);

            Response.Redirect("SetQP.aspx");
        ...
        protected void btnStatus_Click(object sender, EventArgs e)
        {
            ...
            if (status.CommandName == "MAKE ONLINE")
            {
                ...
                SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["CSddedb"].ToString());
                SqlCommand cmd = new SqlCommand("update DDEQuestionPapers set Status=@Status where ExamCode='" + ddlistExam.SelectedItem.Value + "' and PaperCode='" + status.CommandArgument + "' ", con);
