...
{
    public partial class PublishMarkSheetBySC2 : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Authorisation.authorised(Convert.ToInt32(Session["ERID"]), 112))
            {
                ...
                if (!IsPostBack)
                {
                    ...
                    populateStudents(Session["Students"].ToString());
                    populateMarks();
                    populateMSSNO();
            ...
            try
            {
                ...
                foreach (DataListItem dli in dtlistMS.Items)
                {
                    ...
                    if (FindInfo.IsMSAlreadyPrinted(Convert.ToInt32(srid.Text), Convert.ToInt32(year.Text), Session["ExamCode"].ToString(), Session["MOE"].ToString(), out counter, out times, out lastprinted))
                    {
                        ...
                        Exam.updateMSPrintRecord(Convert.ToInt32(srid.Text), Convert.ToInt32(year.Text), Convert.ToInt32(lfmmarks.Text), Convert.ToInt32(lfomarks.Text), qs, Session["ExamCode"].ToString(),Convert.ToInt32(lblCounter.Text),"N", Session["ResultType"].ToString());
                    ...
                    else
                    {
                        ...
                        Exam.updateMSPrintRecordAndCounter(Convert.ToInt32(srid.Text), Convert.ToInt32(year.Text), Convert.ToInt32(lfmmarks.Text), Convert.ToInt32(lfomarks.Text), qs, Session["ExamCode"].ToString(), counter + 1, "N", Session["ResultType"].ToString(), Convert.ToInt32(Session["ERID"]));
        ...
        private void populateMarks()
        {
            ...
            foreach (DataListItem dli in dtlistMS.Items)
            {
                ...
                string sess = "";
                if (Session["ExamCode"].ToString() == "A15" || Session["ExamCode"].ToString() == "B15")
                {
                    sess = ss.Text;
                }
                else
                {
                    if ((eno.Text.Substring(0, 3) == "A13" || eno.Text.Substring(0, 3) == "C14") && (cid.Text == "12" || cid.Text == "27"))
                    {
                        sess = "A 2013-14";
                    }
                    else
                ...
                {
                    if ((eno.Text.Substring(0, 3) == "C13" || eno.Text.Substring(0, 3) == "C14") && (year.Text == "2") && (FindInfo.findCourseShortNameByID(Convert.ToInt32(cid.Text)) == "MBA"))
                    {
                        sysession = "A 2013-14";
                    }
                    else if ((eno.Text.Substring(0, 3) == "A13" || eno.Text.Substring(0, 3) == "C14") && (cid.Text == "12" || cid.Text == "27"))
                    {
                        sysession = "A 2013-14";
                    }
                    else
                ...
                SqlCommand cmd1 = new SqlCommand("select * from DDEPractical where SyllabusSession='"+sysession+"' and CourseName='" + FindInfo.findCourseNameBySRID(Convert.ToInt32(srid.Text), Convert.ToInt32(year.Text)) + "' and Year='" + FindInfo.findAlphaYear(year.Text) + "'", con1);
                SqlDataReader dr1;
        ...
        private void FillSubjectMarks(int srid, int subid, DataRow drow)
        {
            ...
            if (dr.HasRows)
            {
                ...
                {
                    drow["Theory"] = ((Convert.ToInt32(dr["Theory"]) * 60) / 100).ToString();
                    if (Session["cstatus"].ToString() == "NotSet")
                    {
                        Session["cstatus"] = "Complete";
                    }
        ...
        private void populateStudents(string students)
        {
            ...
            DataColumn dtcol7 = new DataColumn("EnrollmentNo");
            DataColumn dtcol8 = new DataColumn("Examination");
            ...
            for (int i = 0; i < srid.Length; i++)
            {
                ...
                while (dr.Read())
                {
                    string year = FindInfo.findAllExamYear(Convert.ToInt32(srid[i]), Session["ExamCode"].ToString(), "R");
                    if (year != "NF" && year != "0" && year != "")
                    {

                        DataRow drow = dt.NewRow();
                        drow["SRID"] = Convert.ToInt32(srid[i]);
                        ...
                        drow["SCCode"] = FindInfo.findSCCodeForMarkSheetBySRID(Convert.ToInt32(srid[i]));
                        drow["RollNo"] = FindInfo.findRollNoBySRID(Convert.ToInt32(srid[i]), Session["ExamCode"].ToString(), "R");
                        drow["EnrollmentNo"] = Convert.ToString(dr["EnrollmentNo"]);
                        drow["Examination"] ="Examination : "+ Session["ExamName"].ToString();
                        int ye = 0;
                        if (year.Length == 1)
                        {
                            ye = Convert.ToInt32(year);
                        }
                        else if (year.Length == 3)
                        {
                            ye = Convert.ToInt32(year.Substring(2, 1));
                        }
                        else if (year.Length == 5)
                        {
                            ye = Convert.ToInt32(year.Substring(4, 1));
                        }
                        drow["Year"] = ye;
                        ...
                        {
                            drow["Course"] = FindInfo.findCourseFullNameBySRID(Convert.ToInt32(srid[i]), Convert.ToInt32(drow["Year"])) + " - FINAL YEAR";
                        }
                        else
                        {

                            drow["Course"] = FindInfo.findCourseAndYearForNewMS(Convert.ToInt32(srid[i]), Convert.ToInt32(drow["Year"]));
