...
{
    public partial class UploadInternalMarks : System.Web.UI.Page
    {
        ...
        {
            if (Session["ExamCode"].ToString() == "B13" || Session["ExamCode"].ToString() == "A14" || Session["ExamCode"].ToString() == "B14" || Session["ExamCode"].ToString() == "A15" || Session["ExamCode"].ToString() == "B15" || Session["ExamCode"].ToString() == "A16" || Session["ExamCode"].ToString() == "B16" || Session["ExamCode"].ToString() == "A17" || Session["ExamCode"].ToString() == "B17" || Session["ExamCode"].ToString() == "A18" || Session["ExamCode"].ToString() == "B18" || Session["ExamCode"].ToString() == "W10")
            {
                if (Session["ResultType"].ToString() == "R")
                {
                    ...
                    DataColumn dtcol6 = new DataColumn("MIA");
                    DataColumn dtcol7 = new DataColumn("MAW");
                    DataColumn dtcol8 = new DataColumn("IA");
                    DataColumn dtcol9 = new DataColumn("AW");
        ...
        protected void btnUpload_Click(object sender, EventArgs e)
        {
                ...
                foreach (DataListItem dli in dtlistSubMarks.Items)
                {
                    ...
                    if (lblmf.Text == "False")
                    {
                        if (ia.Text != "" || aw.Text != "")
                        {
                            ...
                            Log.createLogNow("Marks Filling", "Filled '" + ia.Text + "' IA & '" + aw.Text + "' AW internal marks of a student with Enrollment No '" + lblENo.Text + "' of subject '" + FindInfo.findSubjectDetailByID(Convert.ToInt32(lblsub.Text)) + "' for " + Session["ExamName"].ToString() + " exam", Convert.ToInt32(Session["ERID"].ToString()));
                        }
                    }
                    ...
                    else if (lblmf.Text == "True")
                    {
                        ...
                        {
                            SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings["CSddedb"].ToString());
                            SqlCommand cmd = new SqlCommand("update DDEMarkSheet_" + Session["ExamCode"].ToString() + " set IA=@IA,AW=@AW where SRID='" + lblSRID.Text + "' and SubjectID='" + lblsub.Text + "' and MOE='" + Session["ResultType"].ToString() + "'", con);
                            cmd.Parameters.AddWithValue("@IA", ia.Text);
                            cmd.Parameters.AddWithValue("@AW", aw.Text);
                            ...
                            Log.createLogNow("Marks Updation", "Updated internal marks IA from '" + lia.Text + "' to '" + ia.Text + "' & AW from '" + law.Text + "' to '" + aw.Text + "' of a student with Enrollment No '" + lblENo.Text + "' of subject '" + FindInfo.findSubjectDetailByID(Convert.ToInt32(lblsub.Text)) + "' for " + Session["ExamName"].ToString() + " exam", Convert.ToInt32(Session["ERID"].ToString()));
                        }
                    }
            ...
            else
            {
                ...
                {
                    lblMSG.Text = "Invalid entry at Subject Name - ' " + Session["Sub"].ToString() + " '" + "</br> <div align='left' >Please check : </br> 1. Marks should be in numeric form. </br> 2. Marks should be between 0-16.</br>( If you want to fill 17 to 20 marks than please send the copy of this student to us.</br>The marks will be uploaded by us after checking the copy of student.)</div>";
                }
